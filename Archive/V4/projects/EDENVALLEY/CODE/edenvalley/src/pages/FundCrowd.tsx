...
import { submitBackerApplication } from '@/services/api';
...
const FundCrowd = () => {
  ...
  useEffect(() => {
    document.documentElement.classList.add('fund-crowd-page');
    window.addEventListener('wheel', handleWheel, { passive: false });
    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('touchstart', handleTouchStart, { passive: true });
    window.addEventListener('touchmove', handleTouchMove, { passive: false });
    return () => {
      document.documentElement.classList.remove('fund-crowd-page');
        ...
        <div className={`frame ${fc(0)}`}>
          <div className="max-w-5xl px-4 md:px-12 text-center">
            <p className="font-display text-foreground font-extralight tracking-tight leading-[1.1] text-morph-in mb-4 md:mb-6 px-2 md:px-0" style={{ fontSize: 'clamp(1.5rem, 5vw, 3.5rem)' }}>
              {t('fundCrowd.frame1.headline')}
            </p>
            <p className="font-body text-muted-foreground text-xs md:text-base leading-relaxed max-w-xl md:max-w-3xl mx-auto reveal-up stagger-2 px-2 md:px-0">
              {t('fundCrowd.frame1.body')}
            </p>
          </div>
        </div>

        {/* Frame 1: The Vision — Why Back Early */}
        <div className={`frame ${fc(1)}`}>
          <div className="max-w-4xl px-4 md:px-12">
            ...
            <p className="font-body text-muted-foreground text-xs md:text-base leading-relaxed mb-4 md:mb-6 reveal-up stagger-1 px-2 md:px-0">
              {t('fundCrowd.frame2.body')}
            </p>

            <p className="font-body text-foreground text-xs md:text-base leading-relaxed mb-4 md:mb-8 border-l-2 border-primary/40 pl-4 md:pl-6 reveal-up stagger-2 px-2 md:px-0">
              {t('fundCrowd.frame2.equity')}
            </p>

            <blockquote className="font-display text-foreground text-sm md:text-lg lg:text-xl text-center italic border-t border-b border-primary/20 py-3 md:py-6 reveal-up stagger-3 px-2 md:px-0">
              "{t('fundCrowd.frame2.quote')}"
            </blockquote>
          </div>
        </div>

        {/* Frame 2: Steps + Application Form */}
        <div className={`frame ${fc(2)}`}>
          <div className="max-w-4xl w-full px-4 md:px-8 mx-auto flex flex-col justify-center py-4 md:py-6">
            <div className="text-center mb-4 md:mb-6">
              <p className="font-display text-foreground font-light tracking-tight leading-[1.2] mb-2" style={{ fontSize: 'clamp(1rem, 2.5vw, 1.5rem)' }}>
                {t('fundCrowd.frame3.headline')}
              </p>
            </div>

            {/* Steps */}
            <div className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-3 mb-6 reveal-up">
              {[1, 2, 3, 4, 5].map((num, idx) => (
                <div key={num} className={`flex md:flex-col items-center md:text-center gap-2 md:gap-1 p-2 md:p-3 border border-primary/10 rounded reveal-up stagger-${idx + 1}`}>
                  <span className="font-display text-primary text-lg md:text-xl flex-shrink-0">{num}</span>
                  <p className="font-body text-foreground text-xs md:text-[11px] leading-tight">{t(`fundCrowd.step${num}`)}</p>
                </div>
              ))}
            </div>

            {/* Form */}
            <div className="max-w-md mx-auto w-full pt-3 md:pt-4 border-t border-primary/20">
              <form onSubmit={handleSubmit} className="space-y-2 md:space-y-3 reveal-up">
                <div className="space-y-0.5">
                  <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('fundCrowd.form.name')}</label>
                  <input
                ...
                <div className="space-y-0.5">
                  <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('fundCrowd.form.email')}</label>
                  <input
                    className="w-full px-3 py-1.5 bg-card/50 border border-border text-foreground text-sm outline-none focus:border-primary transition-colors"
                    type="email"
                    value={form.email}
                ...
                <div className="space-y-0.5">
                  <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('fundCrowd.form.amount')}</label>
                  <input
                    className="w-full px-3 py-1.5 bg-card/50 border border-border text-foreground text-sm outline-none focus:border-primary transition-colors"
                    value={form.amount}
                    onChange={(e) => setForm({ ...form, amount: e.target.value })}
                    placeholder={t('fundCrowd.form.amountPlaceholder')}
                    required
                  />
                </div>
