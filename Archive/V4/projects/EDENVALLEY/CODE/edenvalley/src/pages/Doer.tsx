...
import { submitDoerApplication } from '@/services/api';
...
const Doer = () => {
  ...
  useEffect(() => {
    document.documentElement.classList.add('doer-page');
    window.addEventListener('wheel', handleWheel, { passive: false });
    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('touchstart', handleTouchStart, { passive: true });
    window.addEventListener('touchmove', handleTouchMove, { passive: false });
    return () => {
      document.documentElement.classList.remove('doer-page');
  ...
  return (
    <div className="fixed inset-0 bg-background overflow-hidden">
      ...
      <div className="sticky-stage">
        ...
        {/* Frame 0: Hero — The Doer Identity */}
          ...
          <div className="max-w-xl w-full px-4 md:px-8 mx-auto h-full flex flex-col justify-center pt-16 md:pt-20 pb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-2 md:space-y-3 reveal-up">
              <div className="space-y-0.5">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('doer.form.name')}</label>
                <input
              ...
              <div className="space-y-0.5">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('doer.form.email')}</label>
                <input
                  className="w-full px-3 py-1.5 bg-card/50 border border-border text-foreground text-sm outline-none focus:border-primary transition-colors"
                  type="email"
                  value={form.email}
              ...
              <div className="space-y-0.5">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('doer.form.skill')}</label>
                <input
                  className="w-full px-3 py-1.5 bg-card/50 border border-border text-foreground text-sm outline-none focus:border-primary transition-colors"
                  value={form.skill}
                  onChange={(e) => setForm({ ...form, skill: e.target.value })}
                  placeholder={t('doer.form.skillPlaceholder')}
                  required
                />
              </div>

              <div className="space-y-0.5">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('doer.form.shipped')}</label>
                <textarea
                  ...
                  placeholder={t('doer.form.shippedPlaceholder')}
                  required
                />
              </div>

              <div className="space-y-0.5">
                <label className="text-[10px] uppercase tracking-wider text-muted-foreground block">{t('doer.form.vision')}</label>
                <textarea
                  className="w-full px-3 py-1.5 bg-card/50 border border-border text-foreground text-sm outline-none focus:border-primary transition-colors resize-none"
                  rows={1}
                  value={form.vision}
                  onChange={(e) => setForm({ ...form, vision: e.target.value })}
                  placeholder={t('doer.form.visionPlaceholder')}
                />
              </div>
